<template>
  <div class="container-fluid" style="width: 60vw;" v-if="allData">
      <div class="d-flex">
        ...
        <h3 class="mx-auto">Государственные закупки</h3>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      htmlYears: '2019',
      htmlIndex: 'Количество проведенных конкурсов, единиц',
      regions: [],
      years: [],
      allData: [],
      changeData: [],
      indicators: {'Количество проведенных конкурсов, единиц':'numberOfCompetitionsHeld', 'Общая сумма проведенных конкурсов, тыс. тенге':'totalNumberOfCompetitions'},
  ...
  methods: {
    ...
    changeYear(year) {
        this.htmlYears = year
        axios.get('http://localhost:8070/api/statistics/monitoring-sed/goverment-purchases', {
            params: {
                year: year
            }
        }).then(d => {
            this.monthes = d.data.map(d => d.month)
            this.allData = d.data
            let key = this.indicators[this.htmlIndex]
            this.changeData = this.allData.map(d => d[key])
  ...
  created() {
        axios.get('http://localhost:8070/api/statistics/monitoring-sed/goverment-purchases/years')
        .then(arrYears => {
