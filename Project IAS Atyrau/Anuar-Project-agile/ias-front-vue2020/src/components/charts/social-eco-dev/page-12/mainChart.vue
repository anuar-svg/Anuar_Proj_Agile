<template>
  <div class="container-fluid" style="width: 60vw;" v-if="allData">
      <div class="d-flex">
        ...
        <h3 class="mx-auto">Сельское хозяйство</h3>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      htmlYears: '2019',
      htmlIndex: 'Производство мяса, тыс. тонн',
      regions: [],
      years: [],
      allData: [],
      changeData: [],
      indicators: {'Производство мяса, тыс. тонн':'meatProduction', 'Произодство молока, тыс. тонн':'milkProduction', 'Производство яиц, тыс. штук':'eggProduction', 'Численность скота: КРС, тыс. голов':'numberOfCattle', 'Овцы и козы, тыс. голов':'sheepAndGoats', 'Лошади, тыс. голов':'horses', 'Верблюды, тыс. голов':'camels', 'Свиньи, тыс. голов':'pigs', 'Обьем валовой продукции сельского хозяйства, млн. тг':'volumeOfGrossOutputOfAgriculture', 'Обьем производственной продукции растениеводства, млн. тг':'volumeOfProductionOfCropProducts', 'Обьем производственной продукции животноводства, млн. тг':'volumeOfProductionOfLivestockProducts'},
      monthes: []
    };
  },
  methods: {
    ...
    changeYear(year) {
        this.htmlYears = year
        axios.get('http://localhost:8070/api/statistics/monitoring-sed/agricultural-industry', {
            params: {
                year: year
            }
        }).then(d => {
  ...
  created() {
        axios.get('http://localhost:8070/api/statistics/monitoring-sed/agricultural-industry/years')
        .then(arrYears => {
