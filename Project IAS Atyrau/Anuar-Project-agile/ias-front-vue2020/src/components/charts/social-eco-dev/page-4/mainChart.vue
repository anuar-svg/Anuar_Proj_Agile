<template>
  <div class="container-fluid" style="width: 60vw;" v-if="allData">
      <div class="d-flex">
        ...
        <h3 class="mx-auto">Строительство</h3>
      </div>
...
export default {
  ...
  data() {
    return {
      ...
      htmlYears: '2018',
      htmlIndex: 'Обьем строительных работ, млрд. тенге',
      regions: [],
      years: [],
      allData: [],
      changeData: [],
      indicators: {'Обьем строительных работ, млрд. тенге':'scopeOfConstructionWorks', 'Обьем строительных работ (Государственная), млрд. тенге':'scopeOfConstructionWorksStates', 'Обьем строительных работ (Частаная), млрд. тенге':'scopeOfConstructionWorksPrivate', 'Обьем строительных работ (Иностранная), млрд. тенге':'scopeOfConstructionWorksForeign', 'Строительно-монтажные работы, млрд. тенге':'сonstructionAndInstallationWorks', 'Строительство нежилиых зданий, млрд. тенге':'constructionOfNonResidentialBuildings', 'Строительство жилиых зданий, млрд. тенге':'constructionOfResidentialBuildings', 'Капитальный ремонт, млрд. тенге':'overhaul', 'Текущий ремонт, млрд. тенге':'maintenance', 'Прочие строительные работы, млрд. тенге':'otherConstructionWorks', 'Прочие обьекты строительства, млрд. тенге':'otherConstructionProjects'},
  ...
  methods: {
    ...
    changeYear(year) {
        this.htmlYears = year
        axios.get('http://localhost:8070/api/statistics/monitoring-sed/constructions', {
            params: {
                year: year
            }
        }).then(d => {
  ...
  created() {
        axios.get('http://localhost:8070/api/statistics/monitoring-sed/constructions/years')
        .then(arrYears => {
